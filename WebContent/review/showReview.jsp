...
<body>
    ...
    <table class="tablelist" style="width:100%;">
        ...
        <tbody>
        <%
        	List<ArtReview> artRs = (ArrayList<ArtReview>)request.getAttribute("artR");
        	for(ArtReview artR:artRs){
        %>
        <tr style="height:130px;">
        <td class="id"><%=artR.getReview_id() %></td>
        ...
        <%
        	...
        	if(pro>0){
       			%><td>公开</td>
        	<%}else {%><td >私密</td><%} %>
        <td><a href="ArtReviewServlet?oper=del&review_id=<%=artR.getReview_id() %>&art_id=<%=artR.getArt_id()%>" class="tablelink">删除</a>
        </tr> 
        <%} %>
        </tbody>
    </table>
